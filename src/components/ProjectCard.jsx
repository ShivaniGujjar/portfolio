...
import { 
  ...
  SiLangchain, SiGoogle, SiOpenai, SiMongoose
} from 'react-icons/si';
...
const ProjectCard = ({ id, title, description, techStack, imageSrc, repoLink, liveLink }) => {
  return (
    <motion.div 
    ...
    >
      ...
      <div className="px-5 pb-5 pt-1 flex flex-col items-start flex-grow">
        ...
        <div className="grid grid-cols-3 w-full bg-[#0A0A0A] p-3 rounded-xl border border-white/[0.03] gap-2 mb-4">
          {[
            ...
            <div key={i} className="flex flex-col gap-0.5">
              <span className="text-[7px] font-mono font-black tracking-[0.2em] text-[#333] uppercase">{stat.label}</span>
        ...
        <div className="flex gap-2 w-full mt-auto">
          <a href={liveLink} target="_blank" rel="noreferrer" className="flex-1 bg-white/5 border border-white/10 hover:bg-white hover:text-black py-2 rounded-lg font-mono font-bold text-[11px] tracking-widest uppercase flex justify-center items-center gap-2 transition-all">
